...
from db import alterar_produto_db, cadastrar_produto, conectar_db, excluir_produto_db, is_numeric, obter_produto, obter_todos_produtos, obter_todas_vendas_cadastradas
from operations import backup_manual, cadastrar_venda, calcular_total_vendas, obter_vendas_por_mes
...
def cadastrar_venda_ui():
    ...
    produtos_cadastrados = obter_todas_vendas_cadastradas()  # Esta função deve retornar uma lista de nomes de produtos
...
def exportar_vendas_para_csv(vendas, mes):
    try:
        diretorio_documentos = os.path.expanduser('~/Documents/relatorios')
        if not os.path.exists(diretorio_documentos):
            os.makedirs(diretorio_documentos)

        caminho_arquivo = os.path.join(diretorio_documentos, f'relatorio_mes_{mes}.csv')
        ...
        messagebox.showerror("Erro na Exportação", f"Ocorreu um erro ao exportar o relatório: {str(e)}")

def gerar_relatorio_mes(mes_combobox, ano_combobox):
    ...
    if mes in meses_dict:
        ...
        messagebox.showerror("Erro", "Mês inválido selecionado. Por favor, selecione um mês válido.")
        return
    ...
    vendas_mes = obter_vendas_por_mes(mes_numero, ano)

    # Criar uma nova janela para exibir o relatório do mês
    relatorio_window = tk.Toplevel()
    relatorio_window.title(f"Relatório do Mês - {mes} {ano}")

    # Criar uma árvore para exibir as vendas do mês
    tree = ttk.Treeview(relatorio_window, columns=("ID", "Item", "Preço (R$)", "Quantidade", "Tipo de Quantidade", "Tipo de Pagamento", "Data da Venda"), show="headings")
    tree.heading("ID", text="ID")
    tree.heading("Item", text="Item")
    tree.heading("Preço (R$)", text="Preço (R$)")
    ...
    # Adicionar as vendas do mês à árvore
    ...
    # Calcular o fechamento de caixa do mês
    total_vendido = sum(float(venda[2]) for venda in vendas_mes)
    total_quantidade = sum(int(venda[3]) for venda in vendas_mes)

    # Exibir o fechamento de caixa
    fechamento_texto = f"""
    Fechamento de Caixa do Mês:
    Total Vendido: R$ {total_vendido:.2f}
    """
    ...
    def exportar_para_excel():
        ...
        caminho_arquivo = os.path.join(diretorio_documentos, f'relatorio_mes_{mes}_{ano}.xlsx')

        wb = openpyxl.Workbook()
        ws = wb.active
        ws.title = "Relatório de Vendas do Mês"

        headers = ["ID", "Item", "Preço (R$)", "Quantidade", "Tipo de Quantidade", "Tipo de Pagamento", "Data da Venda"]
        ws.append(["Relatório de Vendas do Mês", f"{mes} {ano}"])
        ws.append([])  # Linha em branco
        ws.append(headers)
        ...
        ws.append([])  # Linha em branco
        ws.append(["Resumo", "Valor"])
        ws.append(["Total Vendido no mês", f"R$ {total_vendido:.2f}"])
    ...
    exportar_button = ttk.Button(relatorio_window, text="Exportar para Excel", command=exportar_para_excel)
    exportar_button.pack(pady=10)
...
def visualizar_vendas_por_mes():
    # Função para criar a janela de visualização das vendas por mês
    def exportar_para_xlsx():
        ...
        try:
            ...
            # Adicionar cabeçalhos
            headers = ["ID", "Produto", "Preço (R$)", "Quantidade", "Tipo de Quantidade", "Tipo de Pagamento", "Data da Venda"]
            ws.append(headers)
            ...
            # Salvar o arquivo
            filename = f"vendas_{mes_combobox.get()}.xlsx"
            wb.save(filename)
            messagebox.showinfo("Exportação Concluída", f"Os dados foram exportados para {filename} com sucesso.")
        except Exception as e:
            messagebox.showerror("Erro na Exportação", f"Ocorreu um erro ao exportar os dados: {str(e)}")

    # Função para atualizar a tabela com as vendas do mês selecionado
    def atualizar_tabela():
        for row in tree.get_children():
            tree.delete(row)
        mes_selecionado = mes_combobox.get()
        vendas = obter_vendas_por_mes(mes_selecionado)
        for venda in vendas:
            tree.insert("", "end", values=venda)
        total_vendas = calcular_total_vendas(vendas)
        total_label.config(text=f"Total de Vendas: R$ {total_vendas:.2f}")

    # Criar a janela para visualizar as vendas do mês
    visualizar_vendas_window = tk.Toplevel()
    visualizar_vendas_window.title("Visualizar Vendas por Mês")
    visualizar_vendas_window.geometry("800x400")

    # Criar o frame para os widgets
    frame = tk.Frame(visualizar_vendas_window)
    frame.pack(fill="both", expand=True)

    # Criar o combobox para selecionar o mês
    meses = ["Janeiro", "Fevereiro", "Março", "Abril", "Maio", "Junho", "Julho", "Agosto", "Setembro", "Outubro", "Novembro", "Dezembro"]
    mes_combobox = ttk.Combobox(frame, values=meses, state="readonly")
    mes_combobox.current(datetime.now().month - 1)  # Selecionar o mês atual por padrão
    mes_combobox.grid(row=0, column=0, padx=10, pady=10)

    # Botão para atualizar a tabela
    atualizar_button = ttk.Button(frame, text="Atualizar", command=atualizar_tabela)
    atualizar_button.grid(row=0, column=1, padx=10, pady=10)

    # Botão para exportar para XLSX
    exportar_button = ttk.Button(frame, text="Exportar para XLSX", command=exportar_para_xlsx)
    exportar_button.grid(row=0, column=2, padx=10, pady=10)

    # Criar um treeview para exibir as vendas
    tree = ttk.Treeview(frame, columns=["ID", "Produto", "Preço (R$)", "Quantidade", "Tipo de Quantidade", "Tipo de Pagamento", "Data da Venda"])
    ...
    tree.heading("#3", text="Preço (R$)")
    tree.heading("#4", text="Quantidade")
    tree.heading("#5", text="Tipo de Quantidade")
    tree.heading("#6", text="Tipo de Pagamento")
    tree.heading("#7", text="Data da Venda")
    ...
    # Adicionar label para exibir o total de vendas
    total_label = ttk.Label(frame, text="Total de Vendas: R$ 0.00")
    total_label.grid(row=3, column=0, columnspan=3, padx=10, pady=10)

    # Atualizar a tabela com as vendas do mês atual
    atualizar_tabela()
...
def iniciar_interface():
    ...
    meses = ['Janeiro', 'Fevereiro', 'Março', 'Abril', 'Maio', 'Junho', 'Julho', 'Agosto', 'Setembro', 'Outubro', 'Novembro', 'Dezembro']
    anos = [str(ano) for ano in range(2000, datetime.now().year + 1)]

    mes_label = tk.Label(frame_coluna_esquerda, text="Selecione o Mês:")
    mes_label.pack(pady=5)
    mes_combobox = ttk.Combobox(frame_coluna_esquerda, values=meses)
    mes_combobox.pack(pady=5)

    ano_label = tk.Label(frame_coluna_esquerda, text="Selecione o Ano:")
    ano_label.pack(pady=5)
    ano_combobox = ttk.Combobox(frame_coluna_esquerda, values=anos)
    ano_combobox.pack(pady=5)

    botao_relatorio_mes = ttk.Button(frame_coluna_esquerda, text="Visualizar Relatório do Mês", command=lambda: gerar_relatorio_mes(mes_combobox, ano_combobox), width=30)
    botao_relatorio_mes.pack(pady=10)
